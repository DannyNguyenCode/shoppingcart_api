...
def create_company(db:Session, id:int,name:str)->Company:
    company = db.execute(insert(Company).values(id=id,name=name).returning(
                            Company.id,
                            Company.name
                             )).fetchall()
    db.commit()
...
def update_company(db:Session, id:int,**kwargs):
    company = db.execute(update(Company).where(Company.id == id).values(kwargs).returning(Company.id,Company.name)).fetchall()
    db.commit()
    return company

def delete_company(db:Session,id:int):
    company = db.execute(delete(Company).where(Company.id==id).returning(Company.id,Company.name)).fetchall()
    db.commit()
    return company
...
def create_category(db:Session, id:int,name:str)->Category:
    category = db.execute(insert(Category).values(id=id,name=name).returning(
                            Category.id,
                            Category.name
                             )).fetchall()
    db.commit()
...
def update_category(db:Session,id:int,**kwargs):
    category = db.execute(update(Category).where(Category.id==id).values(kwargs).returning(Category.id,Category.name)).fetchall()
    db.commit()
    return category

def delete_category(db:Session,id:int):
    category = db.execute(delete(Category).where(Category.id==id).returning(Category.id,Category.name)).fetchall()
    db.commit()
    return category
